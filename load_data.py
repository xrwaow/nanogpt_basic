...
def generate_examples(model, checkpoint_path, idx, loss, total_tokens, tokenizer):
    ...
    params = {"temperature": [0.01, 0.5, 1], "min_p": [0.02, 0.02, 0.02]}
    ...
    for example in examples:
        ...
        for i in range(3):
            ...
            ret += f"temp:{params['temperature'][i]:1.2f} | {example}|{tokenizer.decode(out)}\n"
        ret += "---\n"
    ret += "\n"
    
    with open(f"checkpoints/{checkpoint_path}/example_output.txt", "a") as f:
        f.write(ret)